...
function formatArticleMarkdown(text, chapNum, topicNumInChap, hasDiagram) {
  ...
  return `<div class="two-col-flow">${html}</div>`;
}

// Master Book exact CSS
const baseCss = `
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .dummy-offset-page { page-break-after: always; height: 1px; visibility: hidden; }
`;

function buildTopicHtml(t, dummyCount) {
  let offsetPagesHtml = '';
  for (let i = 0; i < dummyCount; i++) {
    offsetPagesHtml += `<div class="page dummy-offset-page"></div>`;
  }
...
async function main() {
  console.log("Generando PDFs Nombra dos como Capitulo_1.1, Capitulo_1.2, etc. con Paginación Continua Exacta...");
  ...
  let runningPageCounter = 3; // Master book topics start at Page 3 (Page 1 = Cover, Page 2 = TOC)
  ...
  for (const cDef of chapterDefinitions) {
    ...
    for (let itemChapIdx = 0; itemChapIdx < cDef.indices.length; itemChapIdx++) {
      ...
      const dummyOffsetCount = runningPageCounter - 1;
      const htmlContent = buildTopicHtml(topicObj, dummyOffsetCount);
      
      const page = await browser.newPage();
      await page.setContent(htmlContent, { waitUntil: 'networkidle0' });

      // First measure exact pages generated by this topic
      ...
      // Count pages in generated buffer
      const pdfText = pdfBuffer.toString('latin1');
      const totalPagesWithDummies = (pdfText.match(/\/Type\s*\/Page\b/g) || []).length;
      const actualTopicPageCount = totalPagesWithDummies - dummyOffsetCount;

      const topicStartPage = runningPageCounter;
      const topicEndPage = topicStartPage + actualTopicPageCount - 1;

      // Save PDF with page range filtering out dummy offset pages using pageRanges
      const pageRangeString = `${dummyOffsetCount + 1}-${totalPagesWithDummies}`;
      ...
      await page.close();
      console.log(`  └─ Topic PDF: ${topicPdfName} (Páginas ${topicStartPage} - ${topicEndPage})`);

      runningPageCounter = topicEndPage + 1;
    }
  }

  await browser.close();
  console.log("\n¡ÉXITO TOTAL! Todos los PDFs Capitulo_X.Y con Paginación Continua e Impecable Estilo del Libro se han generado en:", baseCapitulosDir);
}
