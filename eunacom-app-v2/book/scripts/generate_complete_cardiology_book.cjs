...
const outputPdfPath = path.join(__dirname, 'manual-eunacom-cardiologia-completo-v33.pdf');
...
// Rich, high-value clinical vignettes and in-depth explanations for ALL 47 Cardiology topics
...
// Group 47 Cardiology topics into 5 formal clinical chapters
...
let globalPageNumEstimate = 3;

const allTopicsFormatted = orderedTopics.map((item, seqIdx) => {
  const globalNum = seqIdx + 1;
  const chapNum = item.chapNum;
  const topicNumInChap = item.topicNumInChap;

  const keyPoints = typeof item.rawClass.key_points === 'string' ? JSON.parse(item.rawClass.key_points) : (item.rawClass.key_points || []);
  const rawQuiz = typeof item.rawClass.quiz === 'string' ? JSON.parse(item.rawClass.quiz) : (item.rawClass.quiz || []);
  const diagram = diagramMap[item.originalIndex] || null;

  const rawArticle = item.rawClass.article_content || item.rawClass.clean_transcript || item.rawClass.summary || '';
  const formattedArticle = formatArticleMarkdown(rawArticle, chapNum, topicNumInChap, !!diagram);

  // Extract 3 REAL questions per topic from Question Bank or Class Quiz
  const questionsForTopic = [];
  const qSourceList = [...rawQuiz];

  const topicFirstWord = item.rawClass.topic.toLowerCase().split(' ')[0];
  ...
  for (let qIdx = 0; qIdx < Math.min(3, Math.max(1, qSourceList.length)); qIdx++) {
    ...
    const qText = stripEmojis(qObj.questionText || qObj.pregunta || item.rawClass.topic);
  ...
  const richCasoData = customVignettesMap[item.originalIndex] || {
    vignette: questionsForTopic[0].enunciado,
    explicacion: questionsForTopic[0].explicacionCorrecta
  };

  const topicStartPage = globalPageNumEstimate;
  globalPageNumEstimate += 3;
  ...
  return {
    ...
    title: item.rawClass.topic,
    perfilCode: item.rawClass.eunacom_code || `${chapNum}.01.${topicNumInChap < 10 ? '0' + topicNumInChap : topicNumInChap}`,
    dx: "Específico", tx: "Inicial", seg: "Derivar",
    articleHtml: formattedArticle,
    summaryText: stripEmojis(item.rawClass.summary || ''),
...
function generateHtml() {
  ...
  <div class="page cover">
    <h1>MANUAL EUNACOM CARDIOLOGÍA</h1>
    <h2>Módulo 1 &bull; Todos los 47 Temas de Medicina Interna</h2>
    <div class="badge">Texto Oficial EUNACOM &bull; Chile 2026</div>
  </div>

  <!-- AUTHENTIC CLEAN STEP 2 CK TABLE OF CONTENTS PAGE WITH EXPLICIT BALANCED COLUMNS -->
  ...
  leftChaps.forEach(cNum => {
    ...
    const cTopics = allTopicsFormatted.filter(t => t.chapNum === cNum);
  ...
  pagesHtml += `
        ...
        <a href="#solucionario" class="toc-row-link">
          <div class="toc-row">
            ...
            <span class="toc-page" style="color: #059669;">${globalPageNumEstimate}</span>
          </div>
        </a>
      </div>
    </div>
  ...
  allTopicsFormatted.forEach(t => {
    pagesHtml += `
    <div class="page topic-section" id="tema-${t.globalNum}">
      <!-- TOPIC HEADER (FULL WIDTH) -->
      <div class="topic-hdr">
        <div>
          <div class="num">CAPÍTULO ${t.chapNum} &bull; TEMA ${t.topicLabel}</div>
          <h2>${t.title}</h2>
        </div>
      ...
      ${t.articleHtml}
        ...
        <!-- RICH CASO CLÍNICO TIPO EUNACOM WITH IN-DEPTH CLINICAL EXPLANATION -->
      ...
      <!-- PLANTUML SVG ALGORITHM IF AVAILABLE -->
      ${t.svg ? `
  ...
  // MASTER ANSWER KEY SECTION
  pagesHtml += `
  <div class="page answer-key-page" id="solucionario">
    <div class="toc-page">
      <h2>SOLUCIONARIO OFICIAL DE CARDIOLOGÍA (47 TEMAS)</h2>
      <p style="font-size: 8.5pt; color: #475569; margin-bottom: 12px;">Solucionario oficial de las ${globalQuestionNum - 1} preguntas del Módulo de Cardiología. Incluye fundamento clínico y análisis de opciones.</p>
    </div>
      ...
      pagesHtml += `
        <div class="ans-key-card" id="ans-${q.qSeqNum}">
          <div class="ans-key-hdr">
            <span>RESPUESTA PREGUNTA ${q.qSeqNum} (Cap. ${t.chapNum} - Tema ${t.topicLabel}: ${t.title})</span>
...
<head>
  ...
  <style>
    ...
    * {
    ...
    /* EXACT V21 CLEAN TABLES STYLING */
    .tbl-container {
      width: 100%;
      margin: 6px 0;
      break-inside: avoid;
...
async function main() {
  console.log("Generando MANUAL EUNACOM CARDIOLOGÍA COMPLETO V33 con Casos Clínicos y Preguntas Reales para los 47 Temas...");
  ...
  await page.pdf({
    ...
    headerTemplate: `
      <div style="font-size: 7.5pt; font-family: 'Inter', sans-serif; color: #64748b; width: 100%; display: flex; justify-content: space-between; align-items: center; margin: 0 0.35in; font-weight: 500; text-transform: uppercase; letter-spacing: 0.5px;">
        <span>MANUAL EUNACOM &bull; CARDIOLOGÍA</span>
  ...
  await browser.close();
  console.log("¡ÉXITO! Manual EUNACOM V33 generado en:", outputPdfPath);
}
