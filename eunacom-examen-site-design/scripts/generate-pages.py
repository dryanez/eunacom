...
def drop_block(jsx, marker, signature):
    """Remove the element containing `marker` whose opening line contains
    `signature`, using the emitter's consistent indentation to find its close."""
    lines = jsx.split("\n")
    hit = next((i for i, l in enumerate(lines) if marker in l), None)
    if hit is None:
        raise SystemExit(f"ECOE block not found: {marker!r}")
    start = next(i for i in range(hit, -1, -1) if signature in lines[i])
...
# The academy does not run a practical/ECOE course. Its card and the prose that
# offers it are removed; content that merely explains the exam exists stays.
ECOE_CARD = ('"boxShadow": "0 3px 14px rgba(8,54,95,.07)", "display": "flex", '
             '"flexDirection": "column"}}>')

ECOE_TEXT = [
    ("Nuestro sistema te ayudará a prepararte de manera efectiva para enfrentar tanto el EUNACOM teórico como el EUNACOM práctico en modalidad ECOE.",
     "Nuestro sistema te ayudará a prepararte de manera efectiva para enfrentar el EUNACOM teórico."),
    ("Un curso teórico anual que cubre las dos convocatorias del año, y cursos de seis meses dirigidos al EUNACOM de julio o de diciembre. Además, simulaciones del EUNACOM práctico en modalidad ECOE.",
     "Un curso teórico anual que cubre las dos convocatorias del año, y cursos de seis meses dirigidos al EUNACOM de julio o de diciembre."),
    ("Nuestro sistema te prepara para las dos secciones del examen: el EUNACOM teórico y el EUNACOM práctico en modalidad ECOE.",
     "Nuestro sistema te prepara para el EUNACOM teórico."),
    ('<option value="Curso práctico ECOE">Curso práctico ECOE',
     '<option value="Banco de preguntas">Banco de preguntas'),
]


def drop_ecoe(jsx):
    jsx = drop_block(jsx, "Modalidad ECOE", ECOE_CARD)
    for old, new in ECOE_TEXT:
        if old not in jsx:
            raise SystemExit(f"ECOE copy no longer matches: {old[:60]!r}")
...
def main():
    jsx = drop_ecoe(apply_brand(Path(sys.argv[1]).read_text(encoding="utf-8")))
    keys = None
